        ...
        <div style={{ maxWidth:420, width:'100%' }} className="animate-fade-up">
          ...
          <div style={{ textAlign:'center', marginBottom:28 }}>
            ...
            <p style={{ color:'rgba(253,230,138,0.45)', fontSize:14 }}>
              Start your first entrepreneurial step
            </p>
          </div>
          ...
          <form onSubmit={submit} style={{ padding:24, borderRadius:20, background:'rgba(6,2,24,0.92)', border:'1px solid rgba(245,158,11,0.25)', backdropFilter:'blur(20px)' }}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <input className="input-field" placeholder="e.g. Fatima" value={name} onChange={e => setName(e.target.value)} required autoFocus />
            </div>
            ...
            <div style={{ marginBottom:18 }}>
              ...
              <div style={{ position:'relative' }}>
                <input className="input-field" type={show ? 'text' : 'password'} placeholder="At least 6 characters" value={password} onChange={e => setPassword(e.target.value)} required style={{ paddingRight:42 }} />
              ...
              <p style={{ color:'rgba(253,230,138,0.3)', fontSize:11, marginTop:6 }}>Minimum 6 characters. Hashed before storage.</p>
            </div>
            ...
            <button type="submit" disabled={loading || !name || !email || !password} className="btn-primary"
              style={{ width:'100%', justifyContent:'center', padding:'14px', opacity: (loading || !name || !email || !password) ? 0.5 : 1 }}>
              {loading ? 'Creating account...' : <><UserPlus size={15} /> Create Account <ArrowRight size={14} /></>}
            </button>

            <p style={{ textAlign:'center', marginTop:18, color:'rgba(253,230,138,0.45)', fontSize:13 }}>
              Already have an account?{' '}
              <NavLink to="/login" style={{ color:'#fbbf24', fontWeight:700, textDecoration:'none' }}>
                Sign in
              </NavLink>
            </p>
          </form>
          ...
          <p style={{ textAlign:'center', marginTop:18, color:'rgba(255,255,255,0.12)', fontSize:11 }}>
            🔒 Your account is stored only on this device. No server, no tracking.
          </p>
        </div>
      </div>
    </div>
