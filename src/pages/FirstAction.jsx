...
export default function FirstAction() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => { setResult(getFirstActionResult(form)); setLoading(false) }, 1800)
  }
  ...
  const reset = () => { setResult(null); setDecision(null); setForm(initialForm) }
  ...
  if (result) return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-up">
      ...
      <div>
        ...
        {decision && (
          <div
            className="mt-4 rounded-xl p-4 text-sm text-slate-200 leading-relaxed animate-fade-up"
            ...
            {decision === 'continue' && (
              <div className="mt-3">
                <button onClick={() => navigate('/agents')} className="btn-primary text-sm py-2.5 group">
                  <Users size={14} /> Meet Your AI Team <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            )}
          </div>
    ...
    <div className="max-w-2xl mx-auto animate-fade-up">
      ...
      <form onSubmit={handleSubmit} className="glass p-6 sm:p-8 space-y-6">
        ...
        <div>
          ...
          <input className="input-field" placeholder="e.g. Families in Al Qua'a, local shops, tourists..." value={form.customers} onChange={e => update('customers', e.target.value)} />
        </div>
        ...
        >
          ...
          <p className="text-xs text-slate-500">Your answers stay private and are only used to generate your first action. No data is stored.</p>
        </div>
